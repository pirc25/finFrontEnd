...
import "./chunk-EIDKMX62.js";
import "./chunk-3HPN5KNE.js";
import {
  ListItemText_default
} from "./chunk-IKF2HSSR.js";
import "./chunk-WSLJN6FF.js";
import "./chunk-37VDXUW3.js";
import "./chunk-PEEOWQZN.js";
import {
  Button_default
} from "./chunk-DLN7FV5D.js";
import "./chunk-IKRKEHSH.js";
...
import "./chunk-CRVG4ZR2.js";
import "./chunk-35UNPSDE.js";
import "./chunk-53SUMEJC.js";
import "./chunk-TC7CLAMA.js";
import "./chunk-TUUL2MSK.js";
import "./chunk-3A76C3GX.js";
import "./chunk-EQ2SQOTN.js";
import "./chunk-EXKKR3LC.js";
import {
...
var require_reactMajor = __commonJS({
  "node_modules/@mui/x-internals/reactMajor.js"(exports) {
    ...
    var React156 = _interopRequireWildcard(require_react());
    var _default = exports.default = parseInt(React156.version, 10);
  }
});
...
function fastMemo(component) {
  return React8.memo(component, fastObjectShallowCompare);
}
...
function GridFilterInputValue(props) {
  ...
  const [filterValueState, setFilterValueState] = React12.useState(sanitizeFilterItemValue(item.value, type));
...
var useGridNativeEventListener = (apiRef, ref, eventName, handler, options) => {
  ...
  const handlerRef = React22.useRef(handler);
  const targetElement = isFunction(ref) ? ref() : (ref == null ? void 0 : ref.current) ?? null;
  const wrapHandler = React22.useCallback((event) => {
...
var useRunOnce = (condition, effect) => {
  const didRun = React24.useRef(false);
  useEnhancedEffect_default(() => {
...
var GridHeaderCheckbox = forwardRef2(function GridHeaderCheckbox2(props, ref) {
  ...
  const [, forceUpdate] = React25.useState(false);
  const apiRef = useGridApiContext();
  const rootProps = useGridRootProps();
  const ownerState = {
    classes: rootProps.classes
  ...
  const filteredSelection = React25.useMemo(() => {
  ...
  const selectionCandidates = React25.useMemo(() => {
  ...
  const currentSelectionSize = React25.useMemo(() => filteredSelection.filter((id) => selectionCandidates[id]).length, [filteredSelection, selectionCandidates]);
  ...
  const handleSelectionChange = React25.useCallback(() => {
    forceUpdate((p) => !p);
  }, []);
  React25.useEffect(() => {
...
function GridFilterInputDate(props) {
  ...
  const filterTimeout = useTimeout();
  const [filterValueState, setFilterValueState] = React27.useState(() => convertFilterItemValueToInputValue(item.value, type));
  const [applying, setIsApplying] = React27.useState(false);
  const id = useId();
  const rootProps = useGridRootProps();
...
function GridEditDateCell(props) {
  ...
  const valueTransformed = React28.useMemo(() => {
    let parsedDate;
    if (valueProp == null) {
      parsedDate = null;
  ...
  const [valueState, setValueState] = React28.useState(valueTransformed);
  const rootProps = useGridRootProps();
  const ownerState = {
    classes: rootProps.classes
  };
  const classes2 = useUtilityClasses7(ownerState);
  const parseValueToDate = React28.useCallback((value) => {
    if (value === "") {
      return null;
    }
  ...
  const handleChange = React28.useCallback(async (event) => {
...
function GridEditSingleSelectCell(props) {
  ...
  const apiRef = useGridApiContext();
  const ref = React29.useRef();
  const inputRef = React29.useRef();
  const [open, setOpen] = React29.useState(initialOpen);
  const baseSelectProps = ((_a = rootProps.slotProps) == null ? void 0 : _a.baseSelect) || {};
  const isSelectNative = baseSelectProps.native ?? false;
...
function GridFilterInputSingleSelect(props) {
  ...
  const currentValueOptions = React30.useMemo(() => {
    return getValueOptions(resolvedColumn);
  }, [resolvedColumn]);
  const onFilterChange = React30.useCallback((event) => {
...
function GridFilterInputMultipleSingleSelect(props) {
  ...
  const getOptionValue = resolvedColumn == null ? void 0 : resolvedColumn.getOptionValue;
  const getOptionLabel = resolvedColumn == null ? void 0 : resolvedColumn.getOptionLabel;
  const isOptionEqualToValue = React31.useCallback((option, value) => getOptionValue(option) === getOptionValue(value), [getOptionValue]);
  const resolvedValueOptions = React31.useMemo(() => {
    return getValueOptions(resolvedColumn) || [];
  }, [resolvedColumn]);
  ...
  const handleChange = React31.useCallback((event, value) => {
    applyValue(_extends({}, item, {
...
function GridScrollAreaRaw(props) {
  ...
  const scrollPosition = React32.useRef({
    left: 0,
    top: 0
  });
  ...
  const [canScrollMore, setCanScrollMore] = React32.useState(getCanScrollMore);
  const rootProps = useGridRootProps();
...
var GridPrivateApiContext = React33.createContext(void 0);
if (true) {
  GridPrivateApiContext.displayName = "GridPrivateApiContext";
}
function useGridPrivateApiContext() {
  const privateApiRef = React33.useContext(GridPrivateApiContext);
...
var useGridVirtualScroller = () => {
  ...
  const [panels, setPanels] = React36.useState(EMPTY_DETAIL_PANELS);
  const isRtl = useRtl();
  ...
  const mainRefCallback = React36.useCallback((node) => {
  ...
  const scrollPosition = React36.useRef(EMPTY_SCROLL_POSITION);
  const previousContextScrollPosition = React36.useRef(EMPTY_SCROLL_POSITION);
  const previousRowContext = React36.useRef(EMPTY_RENDER_CONTEXT);
  const renderContext = useGridSelector(apiRef, gridRenderContextSelector);
  const scrollTimeout = useTimeout();
  const frozenContext = React36.useRef(void 0);
  ...
  const focusedCell = {
    rowIndex: React36.useMemo(() => cellFocus ? currentPage.rows.findIndex((row) => row.id === cellFocus.id) : -1, [cellFocus, currentPage.rows]),
    columnIndex: React36.useMemo(() => cellFocus ? visibleColumns.findIndex((column) => column.field === cellFocus.field) : -1, [cellFocus, visibleColumns])
  };
  const updateRenderContext = React36.useCallback((nextRenderContext) => {
  ...
  const scrollerStyle = React36.useMemo(() => ({
  ...
  const contentSize = React36.useMemo(() => {
    const size = {
...
var GridConfigurationContext = React39.createContext(void 0);
if (true) {
...
var useGridConfiguration = () => {
  const configuration2 = React40.useContext(GridConfigurationContext);
...
var GridVirtualScrollbar = forwardRef2(function GridVirtualScrollbar2(props, ref) {
  const apiRef = useGridPrivateApiContext();
  const rootProps = useGridRootProps();
  const isLocked = React47.useRef(false);
  const lastPosition = React47.useRef(0);
  const scrollbarRef = React47.useRef(null);
...
var GridGenericColumnMenu = forwardRef2(function GridGenericColumnMenu2(props, ref) {
  ...
  return (0, import_jsx_runtime53.jsx)(GridColumnMenuContainer, _extends({}, other, {
    ref,
...
function GridColumnsPanel(props) {
  var _a;
  const rootProps = useGridRootProps();
  return (0, import_jsx_runtime55.jsx)(GridPanelWrapper, _extends({}, props, {
    children: (0, import_jsx_runtime55.jsx)(rootProps.slots.columnsManagement, _extends({}, (_a = rootProps.slotProps) == null ? void 0 : _a.columnsManagement))
  }));
}
...
var GridPanel = forwardRef2((props, ref) => {
  ...
  const [isPlaced, setIsPlaced] = React77.useState(false);
  const handleClickAway = React77.useCallback(() => {
    apiRef.current.hidePreferences();
  }, [apiRef]);
  const handleKeyDown = React77.useCallback((event) => {
    if (event.key === "Escape") {
      apiRef.current.hidePreferences();
    }
  }, [apiRef]);
...
function GridPanelContent(props) {
  ...
  return (0, import_jsx_runtime57.jsx)(GridPanelContentRoot, _extends({
    className: clsx_default(classes2.root, className),
    ownerState: rootProps
  }, other));
}
...
function GridPanelFooter(props) {
  ...
  return (0, import_jsx_runtime58.jsx)(GridPanelFooterRoot, _extends({
    className: clsx_default(classes2.root, className),
    ownerState: rootProps
  }, other));
}
...
function GridPanelHeader(props) {
  ...
  return (0, import_jsx_runtime59.jsx)(GridPanelHeaderRoot, _extends({
    className: clsx_default(classes2.root, className),
    ownerState: rootProps
  }, other));
}
...
var GridFilterForm = forwardRef2(function GridFilterForm2(props, ref) {
  ...
  const filterSelectorRef = React81.useRef(null);
  ...
  const sortedFilteredColumns = React81.useMemo(() => {
    switch (columnsSort) {
      case "asc":
  ...
  const currentOperator = React81.useMemo(() => {
    var _a2;
  ...
  const changeColumn = React81.useCallback((event) => {
  ...
  const changeOperator = React81.useCallback((event) => {
  ...
  const changeLogicOperator = React81.useCallback((event) => {
  ...
  React81.useImperativeHandle(focusElementRef, () => ({
    focus: () => {
      var _a2;
  ...
  return (0, import_jsx_runtime60.jsxs)(GridFilterFormRoot, _extends({
    className: classes2.root,
    "data-id": item.id,
    ownerState: rootProps
  }, other, {
    ...
    children: [(0, import_jsx_runtime60.jsx)(FilterFormDeleteIcon, _extends({
    ...
    })), (0, import_jsx_runtime60.jsxs)(FilterFormColumnInput, _extends({
      variant: "standard",
      as: rootProps.slots.baseFormControl
    ...
    })), (0, import_jsx_runtime60.jsxs)(FilterFormOperatorInput, _extends({
      variant: "standard",
      as: rootProps.slots.baseFormControl
      ...
      ownerState: rootProps,
      children: (currentOperator == null ? void 0 : currentOperator.InputComponent) ? (0, import_jsx_runtime60.jsx)(currentOperator.InputComponent, _extends({
        apiRef,
        item,
...
var GridFilterPanel = forwardRef2(function GridFilterPanel2(props, ref) {
  ...
  const lastFilterRef = React82.useRef(null);
  const placeholderFilter = React82.useRef(null);
  const {
    logicOperators = [GridLogicOperator.And, GridLogicOperator.Or],
    columnsSort,
    filterFormProps,
  ...
  const applyFilterLogicOperator = React82.useCallback((operator) => {
    apiRef.current.setFilterLogicOperator(operator);
  }, [apiRef]);
  const getDefaultFilter = React82.useCallback(() => {
  ...
  const getNewFilter = React82.useCallback(() => {
  ...
  const {
    readOnlyFilters,
    validFilters
  } = React82.useMemo(() => items.reduce((acc, item) => {
  ...
  const addNewFilter = React82.useCallback(() => {
  ...
  const deleteFilter = React82.useCallback((item) => {
  ...
  const handleRemoveAll = React82.useCallback(() => {
  ...
  return (0, import_jsx_runtime61.jsxs)(GridPanelWrapper, _extends({}, other, {
    ref,
    children: [(0, import_jsx_runtime61.jsxs)(GridPanelContent, {
      children: [readOnlyFilters.map((item, index) => (0, import_jsx_runtime61.jsx)(GridFilterForm, _extends({
        item,
        ...
        logicOperators,
        columnsSort
      }, filterFormProps), item.id == null ? index : item.id)), validFilters.map((item, index) => (0, import_jsx_runtime61.jsx)(GridFilterForm, _extends({
        item,
        applyFilterChanges: applyFilter,
        deleteFilter,
        hasMultipleFilters,
    ...
    }), !rootProps.disableMultipleColumnsFiltering && !(disableAddFilterButton && disableRemoveAllButton) ? (0, import_jsx_runtime61.jsxs)(GridPanelFooter, {
      children: [!disableAddFilterButton ? (0, import_jsx_runtime61.jsx)(rootProps.slots.baseButton, _extends({
        onClick: addNewFilter,
        startIcon: (0, import_jsx_runtime61.jsx)(rootProps.slots.filterPanelAddIcon, {})
      }, (_a = rootProps.slotProps) == null ? void 0 : _a.baseButton, {
        children: apiRef.current.getLocaleText("filterPanelAddFilter")
      })) : (0, import_jsx_runtime61.jsx)("span", {}), !disableRemoveAllButton && validFilters.length > 0 ? (0, import_jsx_runtime61.jsx)(rootProps.slots.baseButton, _extends({
        onClick: handleRemoveAll,
        startIcon: (0, import_jsx_runtime61.jsx)(rootProps.slots.filterPanelRemoveAllIcon, {})
...
function GridColumnsManagement(props) {
  var _a, _b, _c, _d;
  const apiRef = useGridApiContext();
  const searchInputRef = React83.useRef(null);
  ...
  const isResetDisabled = React83.useMemo(() => checkColumnVisibilityModelsSame(columnVisibilityModel, initialColumnVisibilityModel), [columnVisibilityModel, initialColumnVisibilityModel]);
  const sortedColumns = React83.useMemo(() => {
    switch (sort) {
      case "asc":
  ...
  const currentColumns = React83.useMemo(() => {
  ...
  const toggleAllColumns = React83.useCallback((isVisible) => {
  ...
  const handleSearchValueChange = React83.useCallback((event) => {
    setSearchValue(event.target.value);
  }, []);
  const hideableColumns = React83.useMemo(() => currentColumns.filter((col) => col.hideable), [currentColumns]);
  const allHideableColumnsVisible = React83.useMemo(() => hideableColumns.every((column) => columnVisibilityModel[column.field] == null || columnVisibilityModel[column.field] !== false), [columnVisibilityModel, hideableColumns]);
  const allHideableColumnsHidden = React83.useMemo(() => hideableColumns.every((column) => columnVisibilityModel[column.field] === false), [columnVisibilityModel, hideableColumns]);
  const firstSwitchRef = React83.useRef(null);
  React83.useEffect(() => {
  ...
  const handleSearchReset = React83.useCallback(() => {
    setSearchValue("");
    searchInputRef.current.focus();
  }, []);
  return (0, import_jsx_runtime62.jsxs)(React83.Fragment, {
    children: [(0, import_jsx_runtime62.jsx)(GridColumnsManagementHeader, {
      className: classes2.header,
      ownerState: rootProps,
      children: (0, import_jsx_runtime62.jsx)(SearchInput, _extends({
        ...
        type: "search",
        InputProps: {
          startAdornment: (0, import_jsx_runtime62.jsx)(rootProps.slots.baseInputAdornment, {
            position: "start",
            children: (0, import_jsx_runtime62.jsx)(rootProps.slots.quickFilterIcon, {})
          }),
          endAdornment: (0, import_jsx_runtime62.jsx)(rootProps.slots.baseIconButton, _extends({
            ...
            children: (0, import_jsx_runtime62.jsx)(rootProps.slots.quickFilterClearIcon, {
              fontSize: "small"
            })
          }))
        },
    ...
    }), (0, import_jsx_runtime62.jsxs)(GridColumnsManagementBody, {
      className: classes2.root,
      ownerState: rootProps,
      children: [currentColumns.map((column) => {
        var _a2;
        return (0, import_jsx_runtime62.jsx)(FormControlLabel_default, {
          className: classes2.row,
          control: (0, import_jsx_runtime62.jsx)(rootProps.slots.baseCheckbox, _extends({
      ...
      }), currentColumns.length === 0 && (0, import_jsx_runtime62.jsx)(GridColumnsManagementEmptyText, {
        ownerState: rootProps,
        children: apiRef.current.getLocaleText("columnsManagementNoColumns")
      })]
    }), (!disableShowHideToggle || !disableResetButton) && currentColumns.length > 0 ? (0, import_jsx_runtime62.jsxs)(GridColumnsManagementFooter, {
      ownerState: rootProps,
      className: classes2.footer,
      children: [!disableShowHideToggle ? (0, import_jsx_runtime62.jsx)(FormControlLabel_default, {
        control: (0, import_jsx_runtime62.jsx)(rootProps.slots.baseCheckbox, _extends({
      ...
      }) : (0, import_jsx_runtime62.jsx)("span", {}), !disableResetButton ? (0, import_jsx_runtime62.jsx)(rootProps.slots.baseButton, _extends({
...
var GridToolbarFilterButton = forwardRef2(function GridToolbarFilterButton2(props, ref) {
  ...
  return (0, import_jsx_runtime65.jsx)(rootProps.slots.baseTooltip, _extends({
    ...
    children: (0, import_jsx_runtime65.jsx)(rootProps.slots.baseButton, _extends({
      ...
      "aria-expanded": isOpen,
      "aria-haspopup": true,
      startIcon: (0, import_jsx_runtime65.jsx)(rootProps.slots.baseBadge, _extends({
...
function GridToolbarQuickFilter(props) {
  ...
  React89.useEffect(() => debouncedUpdateSearchValue.clear, [debouncedUpdateSearchValue]);
...
var GridToolbar = forwardRef2(function GridToolbar2(props, ref) {
  ...
  return (0, import_jsx_runtime69.jsxs)(GridToolbarContainer, _extends({}, other, {
    ref,
...
var GridFooter = forwardRef2(function GridFooter2(props, ref) {
  ...
  return (0, import_jsx_runtime71.jsxs)(GridFooterContainer, _extends({}, props, {
    ref,
...
true ? GridRow.propTypes = {
  ...
  dimensions: import_prop_types59.default.shape({
    ...
    groupHeaderHeight: import_prop_types59.default.number.isRequired,
...
function useGridDimensions(apiRef, props) {
  ...
  const pinnedColumns = useGridSelector(apiRef, gridVisiblePinnedColumnDefinitionsSelector);
  const densityFactor = useGridSelector(apiRef, gridDensityFactorSelector);
  const validRowHeight = React138.useMemo(() => getValidRowHeight(props.rowHeight, DATA_GRID_PROPS_DEFAULT_VALUES.rowHeight, rowHeightWarning), [props.rowHeight]);
  ...
  const [savedSize, setSavedSize] = React138.useState();
  const debouncedSetSavedSize = React138.useMemo(() => throttle(setSavedSize, props.resizeThrottleMs), [props.resizeThrottleMs]);
  React138.useEffect(() => debouncedSetSavedSize.clear, [debouncedSetSavedSize]);
  const previousSize = React138.useRef();
...
var useGridColumnResize = (apiRef, props) => {
  ...
  const initialOffsetToSeparator = React143.useRef();
  const resizeDirection = React143.useRef();
  const stopResizeEventTimeout = useTimeout();
  const touchId = React143.useRef();
...
var useGridRowSpanning = (apiRef, props) => {
  ...
  const lastRange = React144.useRef(EMPTY_RANGE);
  const updateRowSpanningState = React144.useCallback(
  ...
  const prevRenderContext = React144.useRef(renderContext);
  const isFirstRender = React144.useRef(true);
  const shouldResetState = React144.useRef(false);
  const previousTree = React144.useRef(tree);
  React144.useEffect(() => {
...
function useProps(allProps) {
  return React153.useMemo(() => groupForwardedProps(allProps), [allProps]);
}
...
var useDataGridProps = (inProps) => {
  ...
  const localeText = React154.useMemo(() => _extends({}, GRID_DEFAULT_LOCALE_TEXT, themedProps.localeText), [themedProps.localeText]);
  const slots = React154.useMemo(() => computeSlots({
    defaultSlots,
    slots: themedProps.slots
  }), [themedProps.slots]);
  const injectDefaultProps = React154.useMemo(() => {
    return Object.keys(DATA_GRID_PROPS_DEFAULT_VALUES).reduce((acc, key) => {
      acc[key] = themedProps[key] ?? DATA_GRID_PROPS_DEFAULT_VALUES[key];
      return acc;
    }, {});
  }, [themedProps]);
  return React154.useMemo(() => _extends({}, themedProps, injectDefaultProps, {
    localeText,
    slots
...
var DataGrid2 = React155.memo(DataGridRaw);
